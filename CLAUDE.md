...
Última sessão fechada: **pt66** (10 Junho 2026 — cirurgia ao watcher HRC: 4 fixes [run intermédia removida → exatamente 2 runs; run-wait robusto; CI **não escrito** + salvaguarda só-leitura; `select_bounty_mode` removido]; backend **911 PASSED** + in-process smoke **ALL OK**; `.exe` **`9ea51ce4…c103bd4`** publicado na Release **`watcher-pt66`**). **⚠️ Fonte pt66 em buffer — push pendente do OK do Rui; re-smoke real pendente.** Journal: `docs/JOURNAL_2026-06-10-pt66.md`. Antecedida por pt62–pt65 (lobby-IT por nome/filename; smoke pt64 **PASSOU** [ciclo HRC ponta-a-ponta] + housekeeping committado `bcd06b7`/`1df3a6c`; bounty-verify pt65). HEAD pushed `1df3a6c`.

⚠️ **Pendente herdado FECHADO:** o smoke real Beelink (pt42d/pt61) **PASSOU em pt64** (`GG-6028190109`, ciclo ponta-a-ponta). O pt66 corrige os defeitos que o smoke pt64 expôs ao vivo (3 runs, run-wait, bounty).

Próxima sessão, por ordem sugerida: 1) **RE-SMOKE pt66** — query distribuição `bountyType`/`progressiveFactor` das mãos elegíveis (após `railway login` do Rui) → escolher **2 mãos** (1 KO não-WN ≠0.5 + 1 não-KO) → re-smoke com o **guia de linhas literais** em `docs/JOURNAL_2026-06-10-pt66.md`; 2) **soltar a fila ~49** (`queue_hold`) **só** após a re-smoke passar + OK do Rui; 3) **recalcular as quarentenas** `GG-6028190109` + `GG-6027751209` (re-POST sobrescreve); 4) **Re-import scope ABRIL**; 5) **`#MYSTERY-KO-DUAL-SUPPORT`** / **`#LOBBY-SYNC-PAGINATION-LIMIT`**. Backlog em `docs/PENDENTES.md`.
