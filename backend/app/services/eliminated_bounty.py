...
def resolve_seat_bounty(
    ...
    *,
    ...
    """CHOKEPOINT do bounty por-seat. Devolve (value, review_reason).

    - Seat NÃO eliminado → devolve a coroa da Vision tal-e-qual (value, None). O caminho
    ...
    - Seat ELIMINADO (HH) → NUNCA a coroa por-seat da Vision. Se houver exatamente 1
      eliminado E 1 verde → value = verde (instantâneo), review None. Senão (0 verdes,
      >1 verde, ou >1 eliminado) → value None + review ('eliminated_no_green'/'ambiguous').
    """
    if name not in busted_names:
        return vision_crown, None
    greens = green_kos or []
    # Caso limpo (SHOULD): 1 eliminado nesta mão + 1 verde → liga-se sem ambiguidade.
    if len(busted_names) == 1 and len(greens) == 1:
        return greens[0]["value"], None
    if not greens:
        return None, REVIEW_NO_GREEN
    return None, REVIEW_AMBIGUOUS
