...
Última sessão fechada: **pt61** (6 Junho 2026 — adaptador HRC instalado no Beelink pela 1ª vez [destrava o smoke pt42d]; smoke de 5 mãos GG **EM CURSO**; única mudança no repo = glossário `68f112f`; HEAD `68f112f`). Journal: `docs/JOURNAL_2026-06-06-pt61.md`. Antecedida por pt60 (`docs/JOURNAL_2026-06-05-pt60.md`).

⚠️ **Pendente herdado:** **Smoke real Beelink pt42d** (`#WN-BOUNTY-NULL-IN-HRC-PIPELINE` v2) — **destravado em pt61** (adaptador instalado); falta **verificar os resultados** do smoke das 5 mãos (EM CURSO) e fechar.

Próxima sessão, por ordem sugerida: 1) **Verificar o smoke pt61** (5 resultados GG + POST dos done + repor `queue_hold`); 2) **Re-import de scope ABRIL** (fechar placeholders Discord/lobby de Abril + `no_match` fora da janela 30/Mai+); 3) **`#MYSTERY-KO-DUAL-SUPPORT`** (🟡 MED); 4) **`#LOBBY-SYNC-PAGINATION-LIMIT`** (🟡 MED); 5) **CoinPoker** (adiado — pacote completo). Backlog em `docs/PENDENTES.md`.
