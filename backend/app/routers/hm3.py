...
def _parse_hand(hh_text, site_name):
    ...
    # ── Date ── (convenção pt51: tudo gravado em hora de Lisboa, naive)
    # PS grava a 1ª timestamp em hora local (WET/Lisboa), seguida do bracket ET;
    # o regex apanha a 1ª (Lisboa). PS/GG já estão em Lisboa → VERBATIM (naive,
    # sem conversão). Winamax/WPN trazem UTC explícito → converter UTC→Lisboa.
    ...
    if m:
        try:
            ...
            if site_name in ("PokerStars", "GGPoker"):
                result["played_at"] = lisbon_naive_verbatim(naive).isoformat()
            else:  # Winamax / WPN — header em UTC → Lisboa
                result["played_at"] = utc_to_lisbon_naive(naive).isoformat()
        except ValueError:
            pass
