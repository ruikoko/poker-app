        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              {level && <span style={{ fontSize: 11, color: '#4b5563', fontFamily: 'monospace', fontWeight: 600 }}>{level}</span>}
            </div>
            {hand.stakes && <div style={{ fontSize: 12, color: '#64748b' }}>{hand.stakes}</div>}
            <div style={{ fontSize: 11, color: '#4b5563', marginTop: 2 }}>{hand.site} &middot; {hand.played_at ? hand.played_at.slice(0, 10) : ''}</div>
          </div>
...
export default function HM3Page() {
  ...
  const [filters, setFilters] = useState({ study_state: '', site: '', search: '', date_from: '' })
  ...
  const load = useCallback(() => {
    setLoading(true)
    setError('')
    const params = { ...filters, page, page_size: 100 }
    // Only show HM3 sites
    if (!params.site) params.search = params.search || ''
    if (!params.date_from) delete params.date_from
    // We need to filter by site on client side if no specific site selected
    hands.list(params)
      .then(d => {
        if (!filters.site) {
    ...
    <>
      ...
      {totalBysite.length > 0 && (
        <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
          {totalBysite.map(s => (
            <div key={s.site} style={{ background: '#1a1d27', border: '1px solid #2a2d3a', borderRadius: 8, padding: '8px 16px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 20, background: '#1a1d27', border: '1px solid #2a2d3a', borderRadius: 10, padding: '12px 16px' }}>
        ...
        {(filters.study_state || filters.site || filters.search || filters.date_from) && (
          <button onClick={() => { setFilters({ study_state: '', site: '', search: '', date_from: '' }); setPage(1) }} style={{ padding: '6px 12px', borderRadius: 6, fontSize: 12, background: 'transparent', color: '#64748b', border: '1px solid #2a2d3a', cursor: 'pointer' }}>&#10005; Limpar</button>
        )}
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #2a2d3a' }}>
                  {['Estado', 'Mão', 'Pos', '+/-', 'Torneio', 'Board', 'Lv', 'Sala', 'Data', 'Tags', ''].map(h => (
                    <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: '#64748b', fontWeight: 600, fontSize: 10, textTransform: 'uppercase', letterSpacing: 0.5, whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((h, idx) => {
                  const level = extractLevel(h.raw)
                  const zebra = idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)'
                  return (
                    <tr key={h.id} style={{ borderBottom: '1px solid #1e2130', cursor: 'pointer', background: zebra }} onClick={() => openDetail(h.id)}
                      onMouseEnter={e => e.currentTarget.style.background = 'rgba(99,102,241,0.04)'}
                      onMouseLeave={e => e.currentTarget.style.background = zebra}>
                      <td style={{ padding: '8px 10px' }}><StateBadge state={h.study_state} /></td>
                      <td style={{ padding: '8px 10px' }}>
                        <div style={{ display: 'flex', gap: 3 }}>
                          {h.hero_cards?.length > 0 ? h.hero_cards.map((c, i) => <PokerCard key={i} card={c} size="sm" />) : <span style={{ color: '#374151' }}>&mdash;</span>}
                        </div>
                      </td>
                      <td style={{ padding: '8px 10px' }}><PosBadge pos={h.position} /></td>
                      <td style={{ padding: '8px 10px' }}><ResultBadge result={h.result} /></td>
                      <td style={{ padding: '8px 10px', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#94a3b8', fontSize: 11 }}>{h.stakes || ''}</td>
                      <td style={{ padding: '8px 10px' }}>
                        <div style={{ display: 'flex', gap: 2 }}>
                          {h.board?.length > 0 ? h.board.slice(0, 5).map((c, i) => <PokerCard key={i} card={c} size="sm" />) : <span style={{ color: '#374151' }}>&mdash;</span>}
                        </div>
                      </td>
                      <td style={{ padding: '8px 10px', fontSize: 10, color: '#4b5563', fontFamily: 'monospace', fontWeight: 600 }}>{level || ''}</td>
                      <td style={{ padding: '8px 10px', fontSize: 10, color: '#4b5563' }}>{h.site === 'Winamax' ? 'WN' : h.site === 'PokerStars' ? 'PS' : h.site === 'WPN' ? 'WPN' : h.site}</td>
                      <td style={{ padding: '8px 10px', fontSize: 10, color: '#4b5563', whiteSpace: 'nowrap' }}>{h.played_at ? h.played_at.slice(0, 10) : ''}</td>
                      <td style={{ padding: '8px 10px' }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>{h.tags?.slice(0, 3).map(t => <Tag key={t} t={t} />)}</div>
                      </td>
                      <td style={{ padding: '8px 10px' }}>
