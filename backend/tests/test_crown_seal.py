...
def test_is_bounty_sealed_predicate():
    ...
    assert is_bounty_sealed({"bounty_source": "cross_exclusion"})  # conflito por exclusão de partes
    ...
    assert {SOURCE_MANUAL, SOURCE_GREEN_KO, SOURCE_DERIVED_GREEN_KO,
            "cross_capture", "cross_conflict", "cross_exclusion"} == set(SEALED_BOUNTY_SOURCES)
