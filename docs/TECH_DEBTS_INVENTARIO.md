...
| **#TABLE-SS-IMAGE-NOT-STORED** | 🟢 LOW | As imagens do table-SS são base64'd só para a chamada Vision e **descartadas** (`table_ss_processing_log` não tem coluna de imagem). Consequência: **sem re-Vision sem re-fornecer o ficheiro**. Foi o que bloqueou validar um prompt Vision multi-site (V2). Aceite como trade-off (poupa storage); registar antes de qualquer plano que dependa de re-Vision retroactiva. |
