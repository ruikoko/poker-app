...
- **Fases 1-3 FEITAS e LIVE** (`d9c504f` leitores → `dc20ad1` writer → `2c8fe8a` propagação). A Fase 2
  fecha por desenho a fusão de seats (MaLong07/4321) e a queda de lugares; a Fase 3 propaga nomes fortes
  pelos brancos das mãos tagadas do torneio (`name_propagation.py`, `match_method=hash_propagation_v1`,
  guardas a–d, OCR-merge endurecido) com **quarentena de nomes** na Saúde GG (`NamePropagationPanel`).
  **Core curado**; falta só o wipe+reimport (aceitação). Guarda (b) mínima também no `/set-anon-map` (409).
