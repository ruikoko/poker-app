...
1. Parse do nome do ficheiro — fiável **APENAS** para o **TM/hand-id** (o número antes do timestamp; ex: `2026-03-06_06_02_PM_2,000_4,000(500)_#TM5672663145.png` → `TM5672663145`). A **hora e a data** no nome são do momento do **DOWNLOAD**, não do jogo; as **blinds** no nome **NÃO são de confiança**. Fonte de verdade da data/hora-de-jogo, blinds, stacks e acções = a **HH** (matematicamente exacta). A imagem/Vision serve só para o que a HH não tem: **nicks reais + tag** (via pasta do IT). Ver `#GG-DOWNLOAD-IMG-FILENAME-TIME-AND-BLINDS-UNRELIABLE` + `DESANON_ANATOMIA.md §2`.
