...
| `#GTO-OPEN-SIZE-NOT-PER-POSITION` | ✅ **FECHADO (commit `90c07ad`, pt89)** | ~~**Opens bucketed** → o size REAL do opener propagava-se a todas as posições que partilhavam `SIZES_OPEN_OTHERS` (CO 2.5 → BTN também "abria" 2.5; e o ALLIN do opener curto contaminava os fundos).~~ **Fix:** opens passam a **per-posição** (cada não-blind tem a sua var `SIZES_OPEN_UTG/UTG1/MP/HJ/CO`, como os 3-bets em pt42b); o gerador faz override **só** ao bucket do opener real, as restantes ficam no default do template `[2]`. **Regra do Rui satisfeita:** opener usa o seu size real; posições à frente usam o standard 2 BB; SB usa o seu size de blind. **Smoke do gerador PROVADO (25 Jun, read-only, sem HRC)** — `GG-6084129607` (HJ abre 2.0bb, eff 18.02bb): `SIZES_OPEN_HJ = [2, ALLIN]` (ALLIN confinado ao opener curto) e `SIZES_OPEN_UTG/_MP/_CO = [2]` (fundos 36.9/32.5/49.5bb limpos, sem allin). **Falta só o smoke da âncora no HRC** (mão que flua naturalmente). Ficheiros: `hrc_node_offset.py`, `hrc_script_gen.py`, `mtt_advanced_canonical_2026.js` (+ testes). Ver `JOURNAL_2026-06-25-pt89.md`, `TECH_DEBTS pt89`. |
