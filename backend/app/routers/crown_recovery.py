...
# ── Mass-reads CONFIRMÁVEIS (última peça da limpeza) ──────────────────────────
_BLOCK_SQL = (
    "SELECT h.id, h.hand_id, h.tournament_name AS tname, h.tournament_number AS tn, "
    "       h.played_at, h.all_players_actions AS apa, ts.buy_in_bounty AS bib "
    "FROM hands h JOIN tournament_summaries ts "
    "  ON ts.site='GGPoker' AND ts.tournament_number = h.tournament_number "
    "WHERE h.site='GGPoker' AND h.played_at >= '2026-01-01' "
    "  AND ts.buy_in_bounty IS NOT NULL AND ts.buy_in_bounty > 0 "
    "  AND h.all_players_actions::text LIKE '%bounty_value_usd%'"
)


def _fresh(c: float, B: float) -> bool:
    return abs(c - B) <= max(0.5, 0.02 * B)


def _block_dismissed_set() -> set:
    try:
        _ensure_drops_dismissed()
        rows = query("SELECT hand_id FROM crown_drop_dismissed WHERE player='__block__'")
        return {r["hand_id"] for r in rows}
    except Exception:  # pragma: no cover - defensivo
        return set()


@router.get("/block-misreads")
def crown_block_misreads(current_user=Depends(require_auth_or_api_key)):
    """Mass-reads CONFIRMÁVEIS: uma captura leu o MESMO valor não-fresco em ≥3 seats E ≥1
    desses seats tem uma leitura ANTERIOR mais ALTA no torneio (a trajetória PROVA que o
    bloco está errado — não é coincidência de grelha). Um card por CAPTURA: a imagem 1×, os
    seats do bloco, e qual tem a prova de queda. A correção é o `/set-bounties` (N seats de
    uma vez, selados manual). Read-only; exclui selados e dispensados."""
    from collections import defaultdict
    from app.services.eliminated_bounty import is_bounty_sealed
    rows = query(_BLOCK_SQL)
    traj = defaultdict(list)              # (tn, hash) -> [(played_at, crown)]
    parsed = []
    for r in rows:
        apa = r["apa"] if isinstance(r["apa"], dict) else json.loads(r["apa"] or "{}")
        seats = []
        for k, v in (apa or {}).items():
            ...
            try:
                ...
                bv = None
            seats.append({"key": k, "nick": v.get("real_name") or k,
                          "position": v.get("position"), "bv": bv,
                          "sealed": is_bounty_sealed(v)})
            if bv is not None and bv > 0:
                traj[(r["tn"], k)].append((r["played_at"], bv))
        parsed.append({"id": r["id"], "hand_id": r["hand_id"], "tname": r["tname"],
                       "tn": r["tn"], "played_at": r["played_at"],
                       "B": float(r["bib"]) / 2.0, "seats": seats})
    dismissed = _block_dismissed_set()
    cards = []
    for p in parsed:
        if p["hand_id"] in dismissed:
            continue
        byval = defaultdict(list)
        for s in p["seats"]:
            if s["bv"] is not None and s["bv"] > 0:
                byval[s["bv"]].append(s)
        for val, members in byval.items():
            if len(members) < 3 or _fresh(val, p["B"]):
                continue
            if all(m["sealed"] for m in members):
                continue
            ...
            if any_proof:
                cards.append({"hand_db_id": p["id"], "hand_id": p["hand_id"],
                              ...
                              "block_value": val, "seats": out_seats})
    cards.sort(key=lambda c: c["hand_id"])
    return {"cards": cards, "count": len(cards)}


@router.post("/block-misreads/dismiss")
def crown_block_dismiss(payload: dict = Body(...),
                        current_user=Depends(require_auth_or_api_key)):
    """Dispensar um card de mass-read (bloco legítimo). Marca a CAPTURA (`player='__block__'`).
    Body: {hand_id}."""
    hand_id = (payload or {}).get("hand_id")
    if not hand_id:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("INSERT INTO crown_drop_dismissed (hand_id, player) VALUES (%s,'__block__') "
