...
# Para hands GG 2026+ que cumprem regra A∨B∨C mas não têm rows em hand_villains
# (bug dormente no live path: /mtt/rematch até 0b01d11 só escrevia em
# villain_notes; e auto-rematch em import_.py/hm3.py usa entry_type='screenshot'
# que não apanha Discord replayer_link/image).
#
# Remover este endpoint após cohort processada e validada.
...
def backfill_missing_hand_villains(
    ...
    ON CONFLICT no INSERT (commit 0b01d11) garante idempotência.
