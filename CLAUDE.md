...
**pt67 — 3 fixes IMPLEMENTADOS (em buffer + Release `watcher-pt67`; re-smoke real PENDENTE):** **#HRC-MAX-PLAYERS-SPAN-NOT-PARTICIPANTS** (✅ **backend** `derive_max_players` = span âncora→BB, **teto 6** [LEI `REGRAS_NEGOCIO §15`]; GG-6029013400 agora 5; 916 PASSED); **#HRC-RUN-WINDOW-DETECTION-BLIND** (✅ watcher: removido o sleep(30) cego; vigia desde o Finish por hwnd, grace de ecrã limpo, zero heurísticas de tempo); **#HRC-CI-SAFEGUARD-CHILD-CONTROLS** (✅ watcher: lê o "Target CI" nos child controls). + In-hand `\S+`→`.+?`. `.exe` **`a9554427`**. **Regra operacional:** NUNCA "Always run in background" no HRC (runbook §2.7).

⚠️ **FILA (~49) TRAVADA** até a **re-smoke pt67 passar** (= as **mesmas 2 mãos**: `GG-6029013400` deve sair com **Max=5** + 1ª run DETECTADA; `GG-6039094225` regressão). **DELETE dos 2 hrc_jobs (job 6,7) FEITO** → as 2 mãos voltaram a elegíveis (recalc da quarentena dessas 2). Restam `GG-6028190109` + `GG-6027751209` (recalc no 1º lote). Requeue no Beelink via `requeue_pt67.bat` (asset da Release; self-fetch do `requeue_state.py`).

Próxima sessão: 1) **re-smoke pt67** das 2 mãos (Beelink: instala → requeue.bat → adapter → watcher; critérios no journal pt67); 2) **soltar a fila** em lotes ao ritmo do Rui (só após passar); 3) recalc `GG-6028190109` + `GG-6027751209`; 4) calibrar `_RUN_WAIT_GRACE_CLEAN_S` se preciso; 5) backlog: Re-import ABRIL, `#MYSTERY-KO-DUAL-SUPPORT`, prova bounty ≠50% deferida. Detalhe: `docs/JOURNAL_2026-06-10-pt67.md` + `PENDENTES.md`.
