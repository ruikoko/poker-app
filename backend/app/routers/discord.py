...
# TEMP: remove after Discord pipeline Opção X validation
@router.post("/admin/test-apply-channel-tags")
def admin_test_apply_channel_tags(
    entry_id: int,
    confirm: bool = False,
    current_user=Depends(require_auth),
):
    """
    Dry-run / wet-run de _apply_channel_tags para uma entry Discord existente.
    Valida pos-deploy da Opcao X sem esperar mensagem nova em producao.

    ?entry_id=X   — entry Discord (source='discord') existente em entries.
    ?confirm=true — executa o UPDATE. Sem este, so mostra preview.

    Response inclui: channel_name, tags derivadas, discord_tags brutos,
    hands afectadas (actual + target) para validacao visual.
    """
    from app.discord_bot import _channel_to_tags, _apply_channel_tags

    # 1. Fetch entry
    entry_rows = query(
        """SELECT id, source, discord_channel, discord_author, discord_message_id
           FROM entries WHERE id = %s""",
        (entry_id,)
    )
    if not entry_rows:
        raise HTTPException(status_code=404, detail=f"entry id={entry_id} nao encontrada")
    entry = entry_rows[0]
    if entry["source"] != "discord":
        raise HTTPException(status_code=400, detail=f"entry {entry_id} nao e source=discord")

    # 2. Resolver channel_name via discord_sync_state
    channel_id = entry["discord_channel"]
    ch_rows = query(
        "SELECT channel_name FROM discord_sync_state WHERE channel_id = %s",
        (channel_id,)
    )
    channel_name = ch_rows[0]["channel_name"] if ch_rows else None
    if not channel_name:
        raise HTTPException(
            status_code=404,
            detail=f"channel_id={channel_id} nao tem channel_name em discord_sync_state",
        )

    tags_derived = _channel_to_tags(channel_name)
    discord_tags_raw = [channel_name] if channel_name else []

    # 3. Snapshot hands antes
    hands_before = query(
        """SELECT id, hand_id, tags, discord_tags, origin
           FROM hands WHERE entry_id = %s""",
        (entry_id,)
    )
    ...
    if not confirm:
        ...
        for h in hands_before:
            cur_tags = list(h["tags"] or [])
            cur_dtags = list(h["discord_tags"] or [])
            new_tags = sorted(set(cur_tags + tags_derived))
            new_dtags = sorted(set(cur_dtags + discord_tags_raw))
            preview.append({
                "id": h["id"],
                "hand_id": h["hand_id"],
                "current_tags": cur_tags,
                "target_tags": new_tags,
                "current_discord_tags": cur_dtags,
                "target_discord_tags": new_dtags,
                "current_origin": h["origin"],
                "target_origin": h["origin"] if h["origin"] else "discord",
            })
        return {
            "mode": "dry_run",
            "entry_id": entry_id,
            "channel_name": channel_name,
            "tags_derived": tags_derived,
            "discord_tags_raw": discord_tags_raw,
            "hands_count": len(hands_before),
            "preview": preview,
            "note": "Adicionar ?confirm=true para executar.",
        }

    # 4. Wet-run: executa via helper real (mesmo path que pipeline live usa).
    _apply_channel_tags(entry_id, tags_derived, channel_name)
    ...
    return {
        ...
        "tags_derived": tags_derived,
        "discord_tags_raw": discord_tags_raw,
        "hands_count": len(hands_after),
        "after": [
            {
                "id": h["id"],
                "hand_id": h["hand_id"],
                "tags": list(h["tags"] or []),
                "discord_tags": list(h["discord_tags"] or []),
                "origin": h["origin"],
            }
            for h in hands_after
        ],
    }
