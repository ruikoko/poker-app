...
⚠️ **A re-smoke pt66 abriu o pt67 (3 achados):** **#HRC-RUN-WINDOW-DETECTION-BLIND** (🔴 o sleep cego de 30s pós-Finish engole runs curtas — a janela da 1ª run é "Hand Setup" do início ao fim, morfa na run); **#HRC-CI-SAFEGUARD-CHILD-CONTROLS** (🟡 o "Target CI" vive num label interior, não no título); **#HRC-MAX-PLAYERS-SPAN-NOT-PARTICIPANTS** (🔴 **backend** `derive_max_players` conta participantes, não o span âncora→BB da regra do Rui [LEI, `REGRAS_NEGOCIO §15`] — `GG-6029013400` saiu Max=2, devia 5). **Regra operacional nova:** NUNCA "Always run in background" no HRC (runbook §2.7).

⚠️ **FILA (~49) TRAVADA** até pt67 + fix Max Players + re-smoke (= as **mesmas 2 mãos**: GG-6029013400 → Max=5 + recalc quarentena; GG-6039094225 → regressão). **Quarentenas:** GG-6028190109 + GG-6027751209 + **GG-6029013400** (recalcular pós-pt67, re-POST sobrescreve).

Próxima sessão (**pt67**), por ordem: 1) **#HRC-MAX-PLAYERS-SPAN-NOT-PARTICIPANTS** (backend, span âncora→BB + testes regra 1 e 2); 2) **#HRC-RUN-WINDOW-DETECTION-BLIND** + **#HRC-CI-SAFEGUARD-CHILD-CONTROLS** (watcher, mesmo build/exe) + recompilar + Release `watcher-pt67`; 3) **re-smoke das 2 mãos** (GG-6029013400 Max=5) + recalc quarentenas; 4) **soltar a fila** em lotes ao ritmo do Rui; 5) backlog: Re-import ABRIL, `#MYSTERY-KO-DUAL-SUPPORT`. Detalhe: `docs/PENDENTES.md` + `TECH_DEBTS_INVENTARIO.md` (pt67).
