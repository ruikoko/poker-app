...
Sessão de **investigação read-only** (zero mudanças de código) sobre os 4 debts
HIGH do resolver/pipelines (3 de pt37 + `#TABLE-SS-RESOLVER-COLLISION` de pt38),
com **queries directas à BD de produção**. Resultado-chave: um dos debts estava
**mal diagnosticado** — ver re-rotulação abaixo. Os outros 3 confirmados
factualmente.
