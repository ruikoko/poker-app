...
def test_relink_recupera_posicoes_do_entry_usa_position_v3():
    """SENTIDO 2 (imagem-primeiro → HH depois): o caller do re-link passa um
    raw_json SEM posições, mas o ENTRY guardado TEM-nas. O enrich recupera-as do
    entry e usa position_v3 — mesmo resultado que o match directo (a ordem não
    importa)."""
    matched_hand = {
        "id": 10, "hand_id": "GG-1", "all_players_actions": {"89ef4cba": {"actions": []}},
        "position": None, "raw": "Poker Hand #TM1: ...", "stakes": "x",
    ...
    # caller passou players_list SEM position (perdeu-as no re-link)
    raw_json = {"hero": "Hero",
                "players_list": [{"name": "Alice", "position": None}, {"name": "Hero", "position": None}],
                "file_meta": {}, "vision_sb": None, "vision_bb": None}
    # entry guardado (id=9) TEM as posições
    entry_rj = {"players_list": [{"name": "Alice", "position": "UTG"}, {"name": "Hero", "position": None}],
                "hero": "Hero"}

    def fake_query(sql, params=None):
        if "FROM entries WHERE id" in sql:
            return [{"raw_json": entry_rj}]
        return [matched_hand]
    ...
    upd = next((p for (s, p) in captured if "UPDATE hands SET player_names" in s), None)
    pn = json.loads(upd[0])
    assert pn["match_method"] == "position_v3", "re-link devia recuperar posições do entry e usar position_v3"
    bypos.assert_called_once()
    bystack.assert_not_called()
