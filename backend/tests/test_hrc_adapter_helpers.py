...
payouts.json on-disk; o adapter chama-o pós-unzip quando o backend
...
from payouts_helpers import rewrite_script_path_in_payouts  # noqa: E402
...
def test_rewrite_script_path_basic():
    ...
    p = _write_temp_json({
        ...
        "max_players": 6,
        "CompletedTournament": {"@flags": "B"},
    })
    ok = rewrite_script_path_in_payouts(p, r"C:\hrc\queue\GG-X\script.js")
    assert ok is True
    ...
    assert data["CompletedTournament"] == {"@flags": "B"}
...
def test_rewrite_script_path_overwrites_existing():
    ...
    ok = rewrite_script_path_in_payouts(p, r"C:\abs\full\path.js")
    assert ok is True
    data = json.loads(p.read_text(encoding="utf-8"))
...
def test_rewrite_script_path_missing_file_returns_false():
    ...
    if missing.exists():
        missing.unlink()
    ok = rewrite_script_path_in_payouts(missing, r"C:\abs.js")
    assert ok is False
...
def test_rewrite_script_path_invalid_json_returns_false():
    ...
    ok = rewrite_script_path_in_payouts(path, r"C:\abs.js")
    assert ok is False
...
def test_rewrite_script_path_non_dict_returns_false():
    ...
    ok = rewrite_script_path_in_payouts(p, r"C:\abs.js")
    assert ok is False
