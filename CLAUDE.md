...
Próxima sessão: 1) **fix `#HRC-NODE-OFFSET-IMPLICIT-LINES`** (modelar o ALLIN implícito; espera confirmação da regra 25 BB do Rui + #400 fora de run); 2) política de **CI/tempo** da 2ª run (`#HRC-2ND-RUN-CI-TIME-DISCREPANCY`); 3) recalc `GG-6028190109`/`GG-6027751209`/#400; 4) **soltar a fila** após o fix da posição; 5) **413 definitivo** (A/B/C; interino 200 MB); 6) rebuild do watcher com o label corrigido (`#HRC-NAV-LABEL-MISLEADING`); 7) backlog: Re-import ABRIL, `#MYSTERY-KO-DUAL-SUPPORT`. Detalhe: `docs/JOURNAL_2026-06-10-pt67.md` + `PENDENTES.md`.
