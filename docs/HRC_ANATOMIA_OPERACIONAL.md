...
REGRAS_NEGOCIO** — desde a 4ª volta pt67 a lei é **LEI B (âncora = a POSIÇÃO certa; a
linha é indiferente)**, confirmada pela semântica de §14.2/§14.3 abaixo.
...
### 14.2 ✅ SEMÂNTICA CONFIRMADA (4ª volta pt67, 3 fotos do Rui — FONTE)

**O desempate empírico correu** (GG-6029013400, âncora R9.01 HJ; 3 fotos: antes / 11 min
/ 78 min). A **observação visual sobrepõe-se a qualquer inferência dos zips**. Veredicto:

> **Um Selected Subtree numa linha recalcula o PONTO DE DECISÃO INTEIRO da posição-âncora
> (TODAS as linhas dela) + TUDO o que está a JUSANTE; as posições ANTERIORES ficam
> congeladas.**

Prova (fotos, decimais):
- **Posição-âncora HJ — todas as linhas mudam:** `HJ 2.00: 5.3% → 1.2%` (esvaziou ≈ só
  AA) e `HJ jam: 17.0% → 20.6%`. Não é a linha-alvo isolada — é a distribuição inteira da
  HJ a redistribuir-se.
- **Jusante muda:** `CO 26.7→30.3`, `CO 0.7→0.0`, `BU 39.2→41.4`, `SB-Complete 61.6→65.0`,
  `SB-Raise 32.5→32.6`.
- **Montante congelado:** `UTG / EP / MP` **idênticos ao decimal**.

**Porquê (raciocínio do Rui — definição, não efeito colateral):** a estratégia numa
decisão é **UMA distribuição única por combo** sobre as acções (fold/sizings). Recalcular
o ponto de decisão **redistribui necessariamente a massa entre TODAS as linhas de sizing
dessa posição** (a HJ 2.00 esvaziou para ≈AA enquanto o jam subiu) — é a **definição de
estratégia mista**, não um spill-over. As posições anteriores ficam congeladas porque as
suas distribuições **não entram no scope** do subtree seleccionado.

**⚠️ Correcção de registo:** a leitura anterior de §14.2 ("irmãos disjuntos; o subtree de
uma linha não toca a irmã; off-by-one = lixo") estava **ERRADA**. Os zips (Complete Export
da árvore inteira, sem marcador de nó-refinado) **não chegavam** para decidir — só a
visão decidiu. Lição: artefacto inconclusivo ⇒ não inferir; medir.

### 14.3 Consequência: âncora = POSIÇÃO certa (LEI B do Rui), a linha é indiferente

Como **qualquer linha da posição-âncora recalcula o mesmo ponto de decisão**, o que
importa é **aterrar na POSIÇÃO certa** — a linha exacta (small-raise vs jam) é
**indiferente** para o âmbito. Logo:

- **Lei B do Rui REACTIVADA e definitiva:** âncora = a **posição-âncora** (mesma da §15:
  regra 1 herói-foldou → herói; regra 2 → 1ª acção voluntária); o 1º nó dessa posição
  serve tão bem como qualquer outro. Alinhado em `REGRAS_NEGOCIO §16`.
- O **off-by-one WITHIN-bucket** (jam vs small-raise, o foco do pt67) é **inofensivo** —
  ambos caem na posição certa. O fix `#HRC-NODE-OFFSET-OFFBY1-REVERT-PT61` (offset 7 vs 6)
...
- **Reavaliar apagados/quarentenados:** os resultados da **3ª volta #400** (âncora R2.00,
  ...
  ser refeito na 4ª volta (resultado equivalente); o #400 entra agora. Rever também as
  quarentenas antigas (`GG-6028190109`, `GG-6027751209`) à luz disto: off-by-one
  within-bucket **não** justifica quarentena; só posição errada justifica.

### 14.4 Observação aberta — CI/tempo da 2ª run (anatomia, a explicar)

Na 4ª volta a 2ª run estava em **CI 121.87 aos 78,5 min** (alvo `<10`), enquanto a **3ª
volta** concluiu âmbito equivalente em **29,5 min**. Discrepância **por explicar**:
