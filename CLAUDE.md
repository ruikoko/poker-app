...
Última sessão fechada: **7–8 Julho 2026 — ★ FRENTE FT CONSTRUÍDA POR INTEIRO (F1–F5) + teste do invariante dos hashes. Tudo LIVE (`main`→`874bd7e`).** Journal: `docs/JOURNAL_2026-07-08.md`.
...
**★ Teste do invariante dos hashes GG (read-only, evidência para o CORE — NÃO APROVADO):** 25 torneios GG 2026 ≥100 mãos (**38 488 aparições**) → **0 violações** do invariante posicional (nenhum hash em 2 mesas à mesma hora); 40 hashes seguem o jogador entre mesas. Os conflitos de nome são **bug da desanon**, não do hash. Caso 293321688: 6 mãos FT sem nomes → **100% resolúveis por propagação de hash** (mas 3 nomes conflituosos → resolver o nome-por-hash primeiro). A decisão do apa-por-hash é do **Rui** (`REGISTO_CONCEITO 2026-07-08`, `JOURNAL_2026-07-08.md`). **F6** (re-solve HRC stale) dorme (0 solves afectados). O **wipe+reimport** é o teste de aceitação da frente FT. **DB reads de prod agora por `~/.pokerapp_db_ro.env`** (fora do repo, só-leitura; `#RAILWAY-TOKEN-SHORT-LIVED`).
