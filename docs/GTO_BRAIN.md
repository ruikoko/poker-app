...
| `#GTO-OPEN-SIZE-NOT-PER-POSITION` | 🟢 FUTURO (p/ riqueza dos dados; **sem fix agora**) | **Opens são bucketed** → o size **REAL** do opener propaga-se a **todas** as posições OTHERS que partilham `SIZES_OPEN_OTHERS` (confirmado: CO abre 2.5 → nas sub-árvores onde o **BTN** é opener, o BTN também "abre" 2.5). **Regra desejada (Rui):** nas posições **à frente** do nó principal (opener), o open deve ser o **standard 2 BB**, não o size real do opener; só o **opener** usa o seu size real; a **SB** usa o seu próprio size de blind (`_blind_open_size_by_eff`); as posições **atrás** do opener (já foldadas) são indiferentes. **Ex.:** CO 2.5 → CO=2.5 (real), BTN=**2.0** (standard), SB=size próprio — hoje o BTN fica 2.5 (errado p/ os dados do GTO Brain). **Por quê futuro:** o Rui não estuda essas ranges hoje; só importa quando o **GTO Brain consumir** estas trees. **Exige tornar os opens PER-POSITION** (como os 3-bets em pt42b) — **trabalho arquitetural, não ajuste**. **NÃO afeta o `#HRC-NODE-OFFSET-IMPLICIT-LINES`** (bug separado; o **valor** do size não gateia o offset, só o **comprimento/ALLIN** do array). Diagnóstico read-only: `hrc_script_gen.py` (`build_sizings_overrides` bc==1; `_array_for_raise` usa `to_amount_bb` real; `_bucket_open_for_position` partilhado). Ver `PENDENTES.md`. |
