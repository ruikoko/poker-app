...
// Defaults null/[] → bloco prune no-op, JS comporta-se idêntico ao original.
let REAL_AGGRESSOR_POS = null;
let DOWNSTREAM_POSITIONS = [];
...
let SIZES_3BET_BB_VS_OTHER = [9, ALLIN];
let SIZES_3BET_SB_VS_BB = [9, ALLIN];
let SIZES_3BET_SB_VS_OTHER = [7, 8, ALLIN];

//special sizing for squeezes in big blinds
let SIZES_3BET_SQUEEZE_IP = [8, ALLIN];
let SIZES_3BET_SQUEEZE_SB = [11, ALLIN];
let SIZES_3BET_SQUEEZE_BB = [12, ALLIN];
let SQUEEZE_INCREASE_PER_CALL = 1.0; //added to squeeze size for multiple callers
...
let POSTFLOP_ADD_FLOP_BET_POT = [0.20]; //Additional pot-bet for all flop bets, e.g. set to 0.33 for a 1/3rd pot bet
...
function getSizingsOpening(ctx) {
	...
	// pt25 prune-in-gap-downstream guard. Quando o backend injecta hints
	// REAL_AGGRESSOR_POS + DOWNSTREAM_POSITIONS, prune branches counter-
	// factuais onde um player downstream do agressor real abre. Estes
	// pathways só existem na árvore HRC porque o solver explora "what if
	// o agressor foldasse"; na mão real não acontecem → impacto zero no
	// EV do spot focal, ganho grande no tamanho da tree.
	// Defaults null/[] → check no-op (idêntico ao original).
	// O check getBetCount === N foi dropped (opção (b) aprovada em pt25):
	// estamos já dentro de getSizingsOpening que só é chamado em opening
	// scenarios; a presença de `player` em DOWNSTREAM_POSITIONS é por si só
	// suficiente para identificar uma branch counterfactual.
	if (REAL_AGGRESSOR_POS !== null
		&& DOWNSTREAM_POSITIONS.length > 0
		&& DOWNSTREAM_POSITIONS.indexOf(player) !== -1) {
		return [];
	}
