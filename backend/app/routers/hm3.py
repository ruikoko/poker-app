...
# TEMP: remove after reparse-buy-in completes and is validated
#
# Scope filter partilhado entre preview e execute. Popula hands.buy_in
# para maos HM3 (2026+) onde esta NULL. Exclui:
#  - WPN: header nao tem padrao numerico extraivel (so prize-pool string).
#  - GG: pipeline diferente (gg_hands.py ja extrai via _extract_buyin_numeric).
_REPARSE_BUYIN_CRITERIA = """
    played_at >= '2026-01-01'
    AND origin = 'hm3'
    AND buy_in IS NULL
    AND site IN ('Winamax', 'PokerStars')
"""


@router.get("/admin/preview-reparse-buy-in")
def admin_preview_reparse_buy_in(current_user=Depends(require_auth)):
    """
    Read-only preview do re-parse de buy_in para maos HM3 (2026+) com
    buy_in NULL em WN/PS. Breakdown por site + sample de 10 ids recentes.
    """
    conn = get_conn()
    try:
        with conn.cursor() as cur:
            cur.execute(f"SELECT COUNT(*) AS n FROM hands WHERE {_REPARSE_BUYIN_CRITERIA}")
            total = cur.fetchone()["n"]

            cur.execute(
                f"""SELECT site, COUNT(*) AS n
                    FROM hands
                    WHERE {_REPARSE_BUYIN_CRITERIA}
                    GROUP BY site ORDER BY n DESC"""
            )
            by_site = [{"site": r["site"], "n": r["n"]} for r in cur.fetchall()]

            cur.execute(
                f"""SELECT id, site, hand_id, played_at, stakes
                    FROM hands
                    WHERE {_REPARSE_BUYIN_CRITERIA}
...
# TEMP: remove after reparse-buy-in completes and is validated
@router.post("/admin/reparse-buy-in")
def admin_reparse_buy_in(
    confirm: str = "",
    current_user=Depends(require_auth),
):
    """
    Re-parse em batch das maos HM3 (2026+) com buy_in NULL (WN/PS).
    Por mao corre _parse_hand(raw, site) e faz UPDATE buy_in = %s.
    NAO mexe em tournament_*, stakes, tags, all_players_actions.
    ...
    try:
        ...
        for hand_id in target_ids:
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        "UPDATE hands SET buy_in = %s WHERE id = %s",
                        (new_buy_in, hand_id),
                    )
                conn.commit()
            except Exception as e:
        ...
        conn.rollback()
        logger.error(f"reparse-buy-in error: {e}")
