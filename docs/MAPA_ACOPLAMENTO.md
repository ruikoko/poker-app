...
**★ Migração da CHAVE do apa (core aprovado 8 Jul — `APA_INDEXACAO_E_COLAPSO §B.6`).** A chave do `all_players_actions` está a mudar de **`real_name`** para a **identidade da HH** (hash na GG; nick real nas outras salas); `real_name` passa a **atributo** (pode estar vazio). Regra de leitura: **`real_name || chave`**. **Fase 1 FEITA e LIVE** (`d9c504f`) — 7 leitores migrados: `villain_rules._build_candidates`, `ire._assemble_ire`, `hand_service._resolve_hashes_in_raw`, `mtt.py` (`seat_to_name`), `hands.py` (filtro `villain`), `table_ss.set_bounties` (`apa_by_name`), frontend `handParser`. Byte-idêntico hoje (premissa `real_name==chave` provada em toda a BD: 68 664 entradas, 0 divergem). **Fases 2 (writer: `_enrich_all_players_actions` deixa de re-indexar) e 3 (propagação por hash + quarentena de nomes) PENDENTES**, com OK do Rui entre cada. **O mapa de acoplamento COMPLETO do apa (~15-18 sítios, writers/readers/frontend) vive em `APA_INDEXACAO_E_COLAPSO §C`** — não duplicar aqui. **Obsoleto quando a Fase 2 entrar:** as afirmações "apa indexado por real_name" espalhadas (ex. comentário em `table_ss.set_bounties`, já corrigido na Fase 1) deixam de valer — a chave passa a ser o hash.
